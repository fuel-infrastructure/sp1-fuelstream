...
use common::mock_tendermint_grpc_server::spawn_grpc_server;
use common::mock_tendermint_rpc_server::spawn_rpc_server;
...
async fn setup_client_with_mocked_servers(
    fixture_name: &'static str,
) -> FuelStreamXTendermintClient {
    let grpc_url = spawn_grpc_server(fixture_name.to_string()).await;
    let rpc_url = spawn_rpc_server(fixture_name.to_string()).await;
...
fn next_light_client_update_succeeds_without_binary_search() {
    run_async_test!(async {
        let mut client =
            setup_client_with_mocked_servers(OVER_66_PERCENT_VOTING_POWER_CHANGE).await;
...
fn next_light_client_update_succeeds_without_binary_search_over_85() {
    run_async_test!(async {
        let mut client =
            setup_client_with_mocked_servers(OVER_85_PERCENT_VOTING_POWER_CHANGE).await;
...
fn fetch_blocks_in_range_succeeds() {
    run_async_test!(async {
        let client = setup_client_with_mocked_servers(OVER_85_PERCENT_VOTING_POWER_CHANGE).await;
