...
macro_rules! run_async_test {
    ($fixture:expr, $test:expr) => {{
        ...
        runtime.block_on(async {
            let grpc_url = spawn_grpc_server($fixture.to_string()).await;
            let rpc_url = spawn_rpc_server($fixture.to_string()).await;
            let client = FuelStreamXTendermintClient::new(rpc_url.parse().unwrap(), grpc_url).await;
