...
impl FuelStreamXOperator {
    ...
    pub async fn run(&mut self) -> Result<Option<(SP1ProofWithPublicValues, B256)>> {
        ...
        // Verify that the bridge commitment generated by the circuit matches the one of the node
        let bridge_commitment = self
            ...
            .await;
        let (_, _, _, _, output_bridge_commitment) =
            ProofOutputs::abi_decode(&proof_output.public_values.to_vec(), true).unwrap();

        assert!(
            bridge_commitment.as_slice() == output_bridge_commitment,
